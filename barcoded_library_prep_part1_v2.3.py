...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p300x8.flow_rate.aspirate = 24  # 25% default flowrate
    p300x8.flow_rate.dispense = 24
    ...
    for column in range(0, 2):
        ...
        sourceLocation = mag_plate.wells()[columnIndex].bottom(0.5).move(types.Point(x=aspirate_x_offset, y=0, z=0))  # Bottom-left (odd columns) or right (even columns)
        p300x8.pick_up_tip()
        p300x8.aspirate(volume=ethanol_volume+10, # Extra volume
                        location=sourceLocation,
                        rate=0.10)  # 10% very slow aspirate speed
        p300x8.air_gap(volume=20)  # 20ul airgap to keep ethanol from dripping
        p300x8.blow_out(location=protocol.fixed_trash['A1'])
    ...
    p300x8.dispense(volume=water_volume,
                    ...
                    rate=1.0)
    p300x8.touch_tip()
    p300x8.blow_out(sourceLocation.top())
    ...
    for column in range(0,2): # Now mix both columns, using currently loaded tips for the first and loading new ones for the second
        ...
        for mix in range(0,5): # 5 Water washes
            p300x8.aspirate(volume=water_volume,
                            location=sourceLocation,
                            rate=0.25) # Slower aspirate to avoid shearing
            p300x8.dispense(volume=water_volume,
                            location=destinationLocation,
                            rate=0.50) # Slightly faster dispense to dislodge magbeads
        p300x8.blow_out(destinationLocation)
        p300x8.drop_tip()
    ...
    for column in range(0,2): # Slowly aspirate eluate from magbeads in each column
        ...
        sourceLocation = mag_plate.wells()[sourceColumnIndex].bottom(0.5).move(types.Point(x=aspirate_x_offset, y=0, z=0)) # Bottom-left (odd columns) or right (even columns)
