# Script name: barcoded_library_prep_part1_v2.2_abbrev.py
# Directory path: cd C:\Users\Max\PycharmProjects\pythonProject\ot2_scripting
# Command line simulation = opentrons_simulate.exe barcoded_library_prep_part1_v2.2_abbrev.py -e

# SECOND VERSION OF SCRIPT, FIRST MAJOR UPDATE 2 (v2.2)
# Handles 12 samples at a time
# Changes made according to OT2 testing on 11/18/2022
...
metadata = {
    'apiLevel': '2.8',
    'protocolName': 'ABBREV Barcoded Library Prep, Part 1 (gDNA repair and end-prep)',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for tube in reagent_tube_list:
        ...
        for well in range(0, 2): # ABBREV 12 TO 2
            ...
            p20x1.dispense(volume=transfer_volume,
                            location=destinationLocation.bottom(3.0),
    ...
    for column in range(0,2):
        ...
        p300x8.mix(repetitions=3,
                ...
                rate=0.5) # 50% flow rate to avoid bubbles
    ...
    # Deactivate temp module til end of protocol run
    ...
    for column in range(0,2):
        ...
        p300x8.aspirate(volume=total_rxn_volume,
                        location=temp_plate.wells()[columnIndex].bottom(0.1),
    ...
    for column in range(0,2):
        ...
        sourceLocation = mag_plate.wells()[columnIndex].bottom(0.1).move(types.Point(x=x_offset, y=0, z=0)) # Bottom left/bottom right well locations
    ...
    for column in range(0,2): # Now mix both columns, using currently loaded tips for the first and loading new ones for the second
        ...
        for mix in range(0,5): # 5 EtOH washes
            ...
            p300x8.dispense(volume=ethanol_volume,
                            location=destinationLocation,
                            rate=0.1) # 10% super gentle drip-like dispense for delicate washing
    ...
    for column in range(0,2):
        ...
        sourceLocation = mag_plate.wells()[columnIndex].bottom(0.1).move(types.Point(x=aspirate_x_offset, y=0, z=0)) # Bottom-left (odd columns) or right (even columns)
        p300x8.pick_up_tip()
        p300x8.aspirate(volume=ethanol_volume,
                        location=sourceLocation,
                        rate=0.10) # 10% very slow aspirate speed
    ...
    for column in range(0,2): # Now mix both columns, using currently loaded tips for the first and loading new ones for the second
        ...
        destinationLocation = mag_plate.wells()[columnIndex].top(-2.0).move(types.Point(x=dispense_x_offset, y=0, z=0)) 
    ...
    for column in range(0,2): # Slowly aspirate eluate from magbeads in each column
        ...
        sourceLocation = mag_plate.wells()[sourceColumnIndex].bottom(0.1).move(types.Point(x=aspirate_x_offset, y=0, z=0)) # Bottom-left (odd columns) or right (even columns)
